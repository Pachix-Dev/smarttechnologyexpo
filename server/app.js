  ...
  try {
    ...
    body.items.forEach((item) => {
      if (ids.includes(item.id)) {
        return res.status(500).send({
          status: false,
          message:
            "Tu compra no pudo ser procesada, la información no es válida...",
        });
      } else {
    ...
    if (limit.length >= 80) {
      return res.status(400).send({
        status: false,
        message:
          "Lo sentimos, hemos llegado al límite de cupo para este evento...",
      });
    }
...
app.post("/expositor-landing-email", async (req, res) => {
  try {
    ...
    const expositorExists = await RegisterModel.check_expositor_exists(
      body.email,
    );
    if (expositorExists) {
      return res.status(400).send({
        status: false,
        message:
          "Ya has registrado una solicitud con este correo electrónico...",
      });
    }
...
app.post("/susbribe-email-ecomondo", async (req, res) => {
  ...
  try {
    ...
    const subscriberExists = await RegisterModel.check_subscriber_exists(
      body.email,
    );
    if (subscriberExists) {
      return res.status(400).send({
    ...
    const userResponse = await RegisterModel.create_suscriber_ecomondo({
      ...body,
    });
...
app.post("/free-register-student-ecomondo", async (req, res) => {
  ...
  try {
    ...
    const userResponse = await RegisterModel.create_user_ecomondo_student({
      ...data,
    });
    ...
    const pdfAtch = await generatePDF_freePass_ecomondo_student(
      body,
      data.uuid,
    );
    const mailResponse = await sendEmailEcomondo_student(
      data,
      pdfAtch,
      data.uuid,
    );
...
app.get("/generate-pdf", async (req, res) => {
  ...
  doc.moveTo(midX, 0).lineTo(midX, doc.page.height).stroke();
  // Horizontal dashed line
  doc.moveTo(0, midY).lineTo(doc.page.width, midY).stroke();
  doc.restore();
  ...
  doc
    ...
    .font("Helvetica")
    .text(
      "Los estudiantes podrán visitar el piso de exposición el viernes 11 de octubre a partir de las 3:00 p.m en Poliforum León.",
      {
        width: 250,
        align: "justify",
      },
  ...
  doc.lineWidth(1);
  doc.moveTo(320, 250).lineTo(600, 250).stroke();
  ...
  doc
    ...
    .font("Helvetica")
    .text(
      " y deberás presentarlo de forma impresa o digital para permitir el ingreso.",
    )
    .moveDown(2);
...
app.get("/template-email-workshop", async (req, res) => {
  ...
  const emailContent = await email_template_workshop({
    ...
    description:
      "Taller práctico donde aprenderás a programar y operar robots colaborativos aplicados a líneas de producción.",
  });
  res.send(emailContent);
});
...
app.post("/workshop-register", async (req, res) => {
  try {
    ...
    if (!Number.isInteger(Number(workshop_id)) || Number(workshop_id) <= 0) {
      return res
        .status(400)
        .send({ status: false, message: "Taller inválido" });
    }
    ...
    if (!workshop)
      return res
        .status(404)
        .send({ status: false, message: "Taller no disponible." });
    ...
    try {
      ...
      const html = await email_template_workshop({
        ...
        description:
          lang === "en" ? workshop.description_en : workshop.description_es,
      });
      ...
      console.log(
        "Aviso: correo no enviado (el registro sí se guardó):",
        mailErr?.message,
      );
    }
    ...
    return res.send({
      status: true,
      message: "Registro a taller confirmado.",
    ...
    return res
      .status(500)
      .send({ status: false, message: "Error al procesar tu registro." });
  }
});

/* Permite enviar correos electrónicos de confirmación de registro */
async function sendEmailEcomondo(
  data,
  pdfAtch = null,
  ...
  try {
    const emailContent =
      data.currentLanguage === "es"
        ? await email_template_ecomondo({ ...data })
        : await email_template_ecomondo_eng({ ...data });
...
async function sendEmailEcomondo_student(
  data,
  pdfAtch = null,
  paypal_id_transaction = null,
) {
  try {
    const emailContent =
      data.currentLanguage === "es"
        ? await email_template_ecomondo_student({ ...data })
        : await email_template_ecomondo_eng_student({ ...data });
